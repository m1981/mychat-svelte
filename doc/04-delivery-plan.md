...
*   ✅ **Auth.js integration:** Google OAuth via `@auth/sveltekit` v5 + `@auth/drizzle-adapter`. `src/auth.ts` + `src/hooks.server.ts`. Layout redirects unauthenticated users to `/auth/signin`; `requireUserId()` helper guards all 14 API routes. DB schema extended with `accounts`, `sessions`, `verificationTokens` tables. 3 E2E smoke tests green.