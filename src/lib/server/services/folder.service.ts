...
import {chatRepository} from '$lib/server/repositories/chat.repository'; // ADDED
...
export class FolderService {
     ...
     */
    async updateFolder(folderId: string, userId: number, data: UpdateFolderDTO & {
        deletedAt?: Date | null
    }): Promise<Folder> {
        // MODIFIED: Pass through deletedAt
        // ... cycle check logic is fine
    ...
    /**
     * Delete a folder (soft or permanent)
     */
    async deleteFolder(folderId: string, userId: number, permanent = false, cascade = false): Promise<void> {
        // MODIFIED: New signature and logic
        const folder = await folderRepository.findById(folderId, userId);
        if (!folder) {
        ...
        if (permanent) {
            // Hard delete
            await folderRepository.delete(folderId, userId);
        } else {
            // Soft delete
            if (!cascade) {
                ...
                // MODIFIED: Move chats to root when soft-deleting
                await chatRepository.unassignFromFolder(folderId, userId);
            }
            await folderRepository.update(folderId, userId, {deletedAt: new Date()});
        }
    }
